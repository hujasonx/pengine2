...
public class PPhysicsCharacterController implements Disposable {
  ...
  private final PVec3 newLinVel = PVec3.obtain();
    ...
    if (rayCast.hasHit()) {
      ...
      if (timeSinceLastJump == -1 || timeSinceLastJump > jumpTimeRequirement) {
        ...
        if (pushUp > 0) {
          ...
          pushUp *= Math.min(1, PEngine.logictimestep * 30); // However, being pushed down to stay on the ground should be almost instantaneous.
        }
