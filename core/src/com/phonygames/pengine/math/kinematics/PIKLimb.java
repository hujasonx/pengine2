...
public class PIKLimb implements PPool.Poolable {
  ...
  public void performIkToReach(float x, float y, float z) {
    ...
    System.out.println("DST:" + pool.vec3().set(endLocalTranslationFromLastNode()).mul(nodes.peek().worldTransform(), 1).dst(t));
...
//      PInverseKinematicsUtils.oneJointRotationToAngleKneeTo(aLRotChange, baseNode.worldTransform().getTranslation(pool.vec3()), kneeNode.worldTransform().getTranslation(pool.vec3()), t,
//                                                            pool.vec3().set(modelSpacePoleTarget)
//                                                                .mul(modelInstance.worldTransform(), 1), aGRot);
//      baseNode.transform().rotate(aLRotChange);
//      baseNode.recalcNodeWorldTransformsRecursive(true);
//    }
    pool.free();
