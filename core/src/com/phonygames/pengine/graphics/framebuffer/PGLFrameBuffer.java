...
public abstract class PGLFrameBuffer<T extends GLTexture> implements Disposable {
   ...
   *
   * <p>
   * FrameBuffers are managed. In case of an OpenGL context loss, which only happens on Android when a user switches
   * to another
   ...
   * the frame buffers
   **/
  protected final static Map<Application, Array<PGLFrameBuffer>> buffers =
      new HashMap<Application, Array<PGLFrameBuffer>>();
  /**
  ...
  protected void build() {
    ...
    if (bufferBuilder.hasPackedStencilDepthRenderBuffer) {
      ...
      gl.glRenderbufferStorage(GL20.GL_RENDERBUFFER, bufferBuilder.packedStencilDepthRenderBufferSpec.internalFormat,
                               width, height);
    }
    ...
    if (result == GL20.GL_FRAMEBUFFER_UNSUPPORTED && bufferBuilder.hasDepthRenderBuffer &&
        bufferBuilder.hasStencilRenderBuffer && (Gdx.graphics.supportsExtension("GL_OES_packed_depth_stencil") ||
  ...
  /**
   * @return The OpenGL handle of the (optional) depth buffer (see {@link GL20#glGenRenderbuffer()}). May return 0
   * even if depth
   * buffer enabled
   */
  ...
  /**
   * @return The OpenGL handle of the (optional) stencil buffer (see {@link GL20#glGenRenderbuffer()}). May return 0
   * even if
  ...
  public static class PFloatFrameBufferBuilder extends PGLFrameBufferBuilder<PFloatFrameBuffer> {
    ...
    @Override public PFloatFrameBuffer build() {
  ...
  public static class PFrameBufferBuilder extends PGLFrameBufferBuilder<PFrameBuffer> {
    ...
    @Override public PFrameBuffer build() {
  ...
  public static abstract class PGLFrameBufferBuilder<U extends PGLFrameBuffer<? extends GLTexture>> {
    ...
    public PGLFrameBufferBuilder<U> addFloatAttachment(int internalFormat, int format) {
      FrameBufferTextureAttachmentSpec spec =
          new FrameBufferTextureAttachmentSpec(internalFormat, format, GL30.GL_FLOAT);
      spec.isFloat = true;
      textureAttachmentSpecs.add(spec);
      return this;
    }
