...
void main() {
    ...
    vec2 rcpFrame = u_sourceTexSize.zw;
    vec2 uv2 = bufferUV * rcpFrame;

    vec4 uv = vec4(bufferUV, bufferUV - (rcpFrame * (0.5 + FXAA_SUBPIX_SHIFT)));
    vec3 col = FxaaPixelShader(uv, u_sourceTex, rcpFrame);
